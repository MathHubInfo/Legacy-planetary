_newModule({
   ...
   validate : (function(){
      ...
      return function( event, container ){
         ...
         for( var i=0; i<max; ++i ){
            obj.push({});
            $.extend( obj[i], tContextMenu.templates.moduleOutput, {
         ...
         obj.push({});
         $.extend( obj[obj.length-1], tContextMenu.templates.moduleOutput, {
            element     : tContextMenu.emptyElement(),
            text        : 'Generator',
            ...
            children    : settingsGenerator( this.options, [['number', 'Min'], ['random', 'Max']])
         });
      ...
      function settingsGenerator( settings, vals ){
         ...
         for( var i in vals ){
            output.push({});
            $.extend( output[i], tContextMenu.templates.moduleOutput, {
      ...
      function settingsInput( opName, label ){
         label = label || opName;
         var elem = tContextMenu.emptyElement()
               .data('opName', opName)
               .append(
