/**
 * @fileOverview Text prefixed search facet functions and UI
 * @author Constantin Jucovschi
 * @author <a href="mailto:jucovschi@gmail.com">Constantin Jucovschi</a>
 */

/**
 * @constructor
 * @class
 * @param {Element} containerElmt
 * @param {Exhibit.UIContext} uiContext
 */
Exhibit.PrefixCompleteFacet = function(containerElmt, uiContext) {
    var self = this;
    Exhibit.jQuery.extend(this, new Exhibit.Facet("text", containerElmt, uiContext));
    this.addSettingSpecs(Exhibit.PrefixCompleteFacet._settingSpecs);

    this._text = null;
    this._dom = null;
    this._timerID = null;
    
    this._onRootItemsChanged = function(evt) {
        if (typeof self._itemToValue !== "undefined") {
            delete self._itemToValue;
        }
    };
    Exhibit.jQuery(uiContext.getCollection().getElement()).bind(
        "onRootItemsChanged.exhibit",
        this._onRootItemsChanged
    );
};

/**
 * @private
 * @constant
 */ 
Exhibit.PrefixCompleteFacet._settingSpecs = {
    "queryParamName":   { "type": "text" },
    "requiresEnter":    { "type": "boolean", "defaultValue": false },
    "onEnter"		:	{ "type": "text", "defaultValue" : "body" }
};

/**
 * @param {Object} configuration
 * @param {Element} containerElmt
 * @param {Exhibit.UIContext} uiContext
 * @returns {Exhibit.PrefixCompleteFacet}
 */
Exhibit.PrefixCompleteFacet.create = function(configuration, containerElmt, uiContext) {
    var uiContext = Exhibit.UIContext.create(configuration, uiContext);
    var facet = new Exhibit.PrefixCompleteFacet(containerElmt, uiContext);
    
    Exhibit.PrefixCompleteFacet._configure(facet, configuration);
    
    facet._initializeUI();
    uiContext.getCollection().addFacet(facet);
    facet.register();
    
    return facet;
};

/**
 * @param {Element} configElmt
 * @param {Element} containerElmt
 * @param {Exhibit.UIContext} uiContext
 * @returns {Exhibit.PrefixCompleteFacet}
 */
Exhibit.PrefixCompleteFacet.createFromDOM = function(configElmt, containerElmt, uiContext) {
    var configuration = Exhibit.getConfigurationFromDOM(configElmt);
    var uiContext = Exhibit.UIContext.createFromDOM(configElmt, uiContext);
    var facet = new Exhibit.PrefixCompleteFacet(
        (typeof containerElmt !== "undefined" && containerElmt !== null) ? containerElmt : configElmt, 
        uiContext
    );
    
    Exhibit.SettingsUtilities.collectSettingsFromDOM(configElmt, facet.getSettingSpecs(), facet._settings);
    
    try {
        var s = Exhibit.getAttribute(configElmt, "expression");
        if (typeof s !== "undefined" && s !== null && s.length > 0) {
            facet.setExpressionString(s);
            facet.setExpression(Exhibit.ExpressionParser.parse(s));
        }
        
        var query = Exhibit.getAttribute(configElmt, "query");
        if (typeof query !== "undefined" && query !== null && query.length > 0) {
            facet._text = query;
        }
    } catch (e) {
        Exhibit.Debug.exception(e, Exhibit._("%facets.error.configuration", "PrefixCompleteFacet"));
    }
    Exhibit.PrefixCompleteFacet._configure(facet, configuration);
    
    facet._initializeUI();
    uiContext.getCollection().addFacet(facet);
    facet.register();
    
    return facet;
};

/**
 * @param {Exhibit.PrefixCompleteFacet} facet
 * @param {Object} configuration
 */
Exhibit.PrefixCompleteFacet._configure = function(facet, configuration) {
    var expression, expressionStrings;
    Exhibit.SettingsUtilities.collectSettings(configuration, facet.getSettingSpecs(), facet._settings);
    
    if (typeof configuration.expression !== "undefined") {
        expression = configuration.expression;
        facet.setExpressionString(expression);
        facet.setExpression(Exhibit.ExpressionParser.parse(expression));
    }
    if (typeof configuration.selection !== "undefined") {
        var selection = configuration.selection;
        for (var i = 0; i < selection.length; i++) {
            facet._valueSet.add(selection[i]);
        }
    }
    if (typeof configuration.query !== "undefined") {
        facet._text = configuration.query;
    }
    if (typeof facet._settings.queryParamName !== "undefined") {
        var params = Exhibit.parseURLParameters();
        if (typeof params[facet._settings.queryParamName] !== "undefined") {
            facet._text = params[facet._settings.queryParamName];
        }
    }
};

/**
 *
 */
Exhibit.PrefixCompleteFacet.prototype.dispose = function() {
    this.getUIContext().getCollection().removeFacet(this);
    Exhibit.jQuery(this.getUIContext().getCollection().getElement()).unbind(
        "onRootItemsChanged.exhibit",
        this._onRootItemsChanged
    );
    
    this._text = null;
    this._dom = null;
    this._itemToValue = null;
    this._timerID = null;

    this._dispose();
};

/**
 * @returns {Boolean}
 */
Exhibit.PrefixCompleteFacet.prototype.hasRestrictions = function() {
    return this._text !== null;
};

/**
 *
 */
Exhibit.PrefixCompleteFacet.prototype.clearAllRestrictions = function() {
    Exhibit.jQuery(this.getContainer()).trigger("onBeforeFacetReset.exhibit");
    var restrictions = this._text;
    if (this._text !== null) {
        this._text = null;
        this._notifyCollection();
    }
    Exhibit.jQuery(this._dom.input).val("");
};

/**
 * @param {Object} restrictions
 * @param {String} restrictions.text
 */
Exhibit.PrefixCompleteFacet.prototype.applyRestrictions = function(restrictions) {
    this.setText(restrictions.text);
};

/**
 * @param {String} text
 */
Exhibit.PrefixCompleteFacet.prototype.setText = function(text) {
    if (typeof text !== "undefined" && text !== null) {
        text = text.trim();
        Exhibit.jQuery(this._dom.input).val(text);
        
        text = text.length > 0 ? text : null;
    } else {
        Exhibit.jQuery(this._dom.input).val("");
        text = "";
    }
    
    if (text !== this._text) {
        this._text = text;
        this._notifyCollection();
    }
};

/**
 * @param {Exhibit.Set} items
 * @returns {Exhibit.Set}
 */
Exhibit.PrefixCompleteFacet.prototype.restrict = function(items) {
    var set, itemToValue, text;
    if (this._text === null || typeof(this._text) == "undefined") {
        return items;
    } else {
        Exhibit.jQuery(this.getContainer()).trigger(
            "onPrefixCompleteFacetSearch.exhibit",
            [ this._text ]
        );
        this._buildMaps();
        
        set = new Exhibit.Set();
        itemToValue = this._itemToValue;
        text = this._text.toLowerCase();
        
        items.visit(function(item) {
            var values, v;
            if (typeof itemToValue[item] !== "undefined") {
                values = itemToValue[item];
                for (v = 0; v < values.length; v++) {
                    if (values[v].indexOf(text) == 0) {
                        set.add(item);
                        break;
                    }
                }
            }
        });
        return set;
    }
};

/**
 * @param {Exhibit.Set} items
 */
Exhibit.PrefixCompleteFacet.prototype.update = function(items) {
    // nothing to do
};

/**
 *
 */
Exhibit.PrefixCompleteFacet.prototype._notifyCollection = function() {
    this.getUIContext().getCollection().onFacetUpdated(this);
};

/**
 *
 */
Exhibit.PrefixCompleteFacet.prototype._initializeUI = function() {
    var self = this;
    this._dom = Exhibit.PrefixCompleteFacet.constructFacetFrame(this.getContainer(), this._settings.facetLabel);

    if (this._text !== null) {
        Exhibit.jQuery(this._dom.input).val(this._text);
    }

    Exhibit.jQuery(this._dom.input).bind("keyup", function(evt) {
        self._onTextInputKeyUp(evt);
    });
    Exhibit.jQuery(this._dom.input).bind("keydown", function(evt) {
        self._onTextInputKeyDown(evt);
    });
};

/**
 * @param {Element} div
 * @param {String} facetLabel
 */
Exhibit.PrefixCompleteFacet.constructFacetFrame = function(div, facetLabel) {
    if (typeof facetLabel !== "undefined"
        && facetLabel !== null
        && facetLabel !== "") {
        return Exhibit.jQuery.simileDOM("string",
            div,
            '<div class="exhibit-facet-header">' +
                '<span class="exhibit-facet-header-title">' + facetLabel + '</span>' +
            '</div>' +
            '<div class="exhibit-text-facet"><input type="text" id="input"></div>'
        );
    } else {
        return Exhibit.jQuery.simileDOM(
            "string",
            div,
            '<div class="exhibit-text-facet"><input type="text" id="input"></div>'
        );
    }
};

/**
* @param {jQuery.Event} evt
*/
Exhibit.PrefixCompleteFacet.prototype._onTextInputKeyDown = function(evt) {
    var self, newText;
    self = this;
	newText = Exhibit.jQuery(this._dom.input).val().trim();
	if (evt.which === 13) {
		window.setTimeout(function() {
                self._onTimeout();
                var restricted = self.getUIContext().getCollection().getRestrictedItems();
                if (restricted.size() == 1) {
                	restricted.visit(function(item) {
                			jQuery(self._settings.onEnter).trigger("onEnter", {"item":item, "db":self.getUIContext().getDatabase()});
                	});
                }
		});		
	}
	if (evt.which === 9) { // arbitrary
		window.setTimeout(function() {
                self._onTimeout();
                var restricted = self.getUIContext().getCollection().getRestrictedItems();
                var maxPrefix = null, maxLength = 0;
                restricted.visit(function(item) {
                		if (maxPrefix == null) {
                			maxPrefix = item;
                			maxLength = maxPrefix.length - 1;
                			return;
                		}
                		var i = Math.min(maxLength, item.length - 1);
                		for (; i>=0; i--) { 
                			if (maxPrefix[i] != item[i])
                			{
                				maxLength = i;
                			}
                		}
                });
                if (maxLength > newText.length) {
                	Exhibit.jQuery(self._dom.input).val(maxPrefix.substr(0, maxLength + 1));
                }
        }, 0);
        evt.preventDefault();
    }
};

/**
 * @param {jQuery.Event} evt
 */
Exhibit.PrefixCompleteFacet.prototype._onTextInputKeyUp = function(evt) {
    var self, newText;
    if (this._timerID !== null) {
        window.clearTimeout(this._timerID);
    }
    self = this;
    
    Exhibit.jQuery(this._dom.input).val()
    
    if (this._settings.requiresEnter === false) {
        this._timerID = window.setTimeout(function() {
            self._onTimeout();
        }, 500);
    } else {
        newText = Exhibit.jQuery(this._dom.input).val().trim(); 
        if (newText.length === 0 || evt.which === 13) { // arbitrary
            this._timerID = window.setTimeout(function() {
                self._onTimeout();
            }, 0);
        }
    }
};


/**
 *
 */
Exhibit.PrefixCompleteFacet.prototype._onTimeout = function() {
    var newText, self, oldText;

    this._timerID = null;
    
    newText = Exhibit.jQuery(this._dom.input).val().trim();
    if (newText.length === 0) {
        newText = null;
    }
    
    if (newText !== this._text) {
        self = this;
        oldText = this._text;
        
        Exhibit.History.pushComponentState(
            this,
            Exhibit.Facet._registryKey,
            { "text": newText },
            newText !== null ?
                Exhibit._("%facets.facetTextSearchActionTitle", newText) :
                Exhibit._("%facets.facetClearTextSearchActionTitle"),
            true
        );
    }
};

/**
 *
 */
Exhibit.PrefixCompleteFacet.prototype._buildMaps = function() {
    console.log("building maps");
    var itemToValue, allItems, database, expression, propertyIDs;
    if (typeof this._itemToValue === "undefined") {
        itemToValue = {};
        allItems = this.getUIContext().getCollection().getAllItems();
        database = this.getUIContext().getDatabase();
        
        expression = this.getExpression();
        allItems.visit(function(item) {
                var values, x;
                values = [];
                expression.evaluateOnItem(item, database).values.visit(function(v) { values.push(v.toLowerCase()); });
                itemToValue[item] = values;
        });
        
        this._itemToValue = itemToValue;
    }
};

/**
 * @returns {Object}
 */
Exhibit.PrefixCompleteFacet.prototype.exportState = function() {
    return this._exportState(false);
};

/**
 * @returns {Object}
 */
Exhibit.PrefixCompleteFacet.prototype.exportEmptyState = function() {
    return this._exportState(true);
};

/**
 * @private
 * @param {Boolean} empty
 @ returns {Object}
 */
Exhibit.PrefixCompleteFacet.prototype._exportState = function(empty) {
    return {
        "text": empty ? null : this._text
    };
};

/**
 * @param {Object} state
 * @param {String} state.text
 */
Exhibit.PrefixCompleteFacet.prototype.importState = function(state) {
    this.applyRestrictions(state);
};
